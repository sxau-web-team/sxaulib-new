...
if($pages>1){
        ...
        echo'<br/><div class="uk-panel uk-width-medium-2-3"><ul class="uk-pagination uk-pagination-center" >';
    
        $current_page=($start/$display)+1;
        if($current_page!=1){

            echo'<li><a href="news.php?s='.($start-$display).'&p='.$pages.'">上一页</a></li>';  
        
        }
    
        if($start-$display>25){
            if($cent+5<=$pages){
                 for($i=$cent-4;$i<=$cent+5;$i++){
                    if($i!=$current_page){  
                        echo '<li><a href="news.php?s='.(($display*($i-1))).'&p='.$pages.'">'.$i.'</a></li>';
                    }else{
        ...
        if($current_page !=$pages){
            echo'<li><a href="news.php?s='.($start+$display).'&p='.$pages.'">下一页</a></li>';
        }
        echo'</ul></div>';
